...
import { Camera, Clock, MapPin, Phone, ArrowLeft } from "lucide-react";
...
export default function CreateRestaurantPage() {
	...
	useEffect(() => {
		api.get("/catalog/restaurants/categories/")
			.then((response) => setExistingCategories(response.data))
			.catch((err) => console.error("Impossible de charger les catégories", err));
	}, []);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			navigate("/admin/restaurants"); // REDIRECTION VERS LA LISTE
		} catch (err) {
		...
		<div className="min-h-screen bg-gray-50 p-6 md:p-10">
			<div className="w-full max-w-[1600px] mx-auto">
				...
				<div className="mb-8 flex justify-between items-start">
					<div>
						<h1 className="text-3xl font-extrabold text-gray-900">Ajouter un restaurant</h1>
						<p className="text-gray-500 mt-2">Configurez la vitrine de votre nouvel établissement</p>
					</div>
					...
					<div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
						...
						<div className="lg:col-span-4 space-y-6 lg:border-l lg:border-gray-100 lg:pl-12 flex flex-col justify-between">
							...
							<div className="pt-8 flex flex-col gap-3">
								<button type="submit" disabled={loading} className={`w-full py-4 text-base font-bold text-white rounded-xl shadow-md transition-all ${loading ? "bg-orange-400" : "bg-orange-600 hover:bg-orange-700"}`}>
									{loading ? "Création en cours..." : "Enregistrer le restaurant"}
								</button>
