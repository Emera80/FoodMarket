...
import { Mail, Search, Trash2, ChevronRight, User, Send, Clock } from 'lucide-react';
...
export default function AdminMessages() {
  ...
  const fetchMessages = async () => {
    ...
    } catch (err) { toast.error("Erreur messages"); } finally { setLoading(false); }
  };
  ...
  const deleteMsg = async (id) => {
    ...
    try {
      await api.post(`/catalog/contact/${selectedMsg.id}/repondre/`, 
        { reponse: replyText }
      );
      toast.success("Réponse envoyée au client !");
      setReplyText(''); // On vide la zone de texte
      setSelectedMsg({...selectedMsg, est_lu: true});
    } catch (err) {
      ...
      <div className="w-1/3 bg-white rounded-[32px] border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        <div className="p-6 border-b border-gray-50 bg-gray-50/50">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Rechercher..." className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
        </div>
        ...
        {selectedMsg ? (
          <>
            <div className="p-8 border-b border-gray-50 flex justify-between items-center">
              ...
              <button onClick={() => deleteMsg(selectedMsg.id)} className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all">
                <Trash2 size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 p-10 overflow-y-auto space-y-6">
              ...
              {/* <div className="pt-10 border-t border-dashed border-gray-200">
                ...
                <div className="relative">
                  <textarea placeholder="Écrivez votre réponse ici..." className="w-full bg-white border border-gray-200 rounded-[24px] p-6 text-sm font-medium h-40 outline-none focus:ring-2 focus:ring-orange-500 shadow-inner"></textarea>
                  <button className="absolute bottom-4 right-4 bg-gray-900 hover:bg-black text-white px-6 py-3 rounded-2xl font-black text-xs uppercase flex items-center gap-2 transition-all">
