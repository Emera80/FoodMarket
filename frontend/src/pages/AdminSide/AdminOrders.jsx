      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 bg-white px-4 py-3 rounded-2xl border border-gray-100 flex items-center gap-3 shadow-sm">
            ...
            <select
              className="flex-1 bg-transparent border-none outline-none font-bold text-sm text-gray-700 cursor-pointer"
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
            >
              <option value="">Tous les statuts</option>
              {Object.entries(STATUTS_CONFIG).map(([key, sc]) => (
          ...
          <div className="flex-1 bg-white px-4 py-3 rounded-2xl border border-gray-100 flex items-center gap-3 shadow-sm">
            ...
            />
            {dateFilter && (
              <button onClick={() => setDateFilter('')} className="p-1 hover:bg-gray-100 rounded-full text-gray-400 hover:text-red-500 transition-colors">
                <X size={14} />
              </button>
            )}
          </div>
      ...
      <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
              ...
              return (
                <tr key={cmd.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => openModal(cmd)} className="p-2 text-gray-400 hover:text-orange-600 transition-colors">
                      <Eye size={20} />
                    </button>
                  </td>
                </tr>
        ...
        {filteredCommandes.length === 0 && (
          <div className="p-12 text-center">
            ...
            <button onClick={resetFilters} className="mt-4 text-orange-600 font-bold text-sm hover:underline">
            ...
            <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 gap-6">
                  {[
                    ['Client', selectedOrder.user_name || 'N/A'],
                    ['Téléphone', selectedOrder.user_phone || 'N/A'],
                    ['Restaurant', selectedOrder.restaurant_nom || 'N/A'],
                    ['Date', new Date(selectedOrder.date).toLocaleString('fr-FR')],
                  ].map(([label, value]) => (
                    <div key={label} className="flex justify-between border-b border-gray-50 pb-2">
                      <span className="text-gray-500 font-bold">{label}</span>
                      <span className={`font-black ${label === 'Restaurant' ? 'text-orange-600' : 'text-gray-900'}`}>{value}</span>
                    </div>
                  ))}
                  <div className="space-y-2">
                    <span className="text-gray-500 font-bold block">Adresse de livraison</span>
                    <p className="bg-gray-50 p-4 rounded-2xl text-gray-900 font-bold text-sm leading-relaxed">
